...
sys.path.append('external/LWTGPF/')
...
class EmbddingComputer:
    ...
    def initialize_model(self):
        if self.dataset=='mot17':
            ...
            if self.test_dataset:
                path = "pretrained/reid/mot20_sbs_S50.pth"
            else:
                return self._get_general_model()
        elif self.dataset == 'dance':
            path = "pretrained/reid/dance_sbs_S50.pth"

        elif self.dataset=='crash':
            return self._get_custom_model() #lwtgpf
        else:
    ...
    def compute_embedding(self,img,box,tag):
        ...
        embs = []
        #todo: 嵌入自制考虑相机标签的ReID模型
        for idx in range(0,len(crops),self.max_batch):
            batch_crops = crops[idx:idx+self.max_batch]
            batch_crops = batch_crops.cuda()
            with torch.no_grad():
                if self.dataset=='crash':
                    batch_embs = self.model(batch_crops,cam_label=torch.as_tensor([0]*batch_crops.size()[0]))
                    if isinstance(batch_embs,dict):
                        batch_embs = batch_embs['global']
                else:
    ...
    def _get_general_model(self):
        ...
        sd = torch.load("pretrained/reid/osnet_ain_ms_d_c.pth.tar")["state_dict"]
    ...
    def _get_custom_model(self):
        ...
        config = 'external/LWTGPF/configs/crash.yaml'
        ckpt_path = 'pretrained/reid/crash_lwtgpf.pt'
